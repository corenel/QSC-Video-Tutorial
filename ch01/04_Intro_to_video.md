...
> 对于NTSC制式来说，每秒59.94场，而PAL制式是每秒60场。在PAL制式和NTSC制式中，都是先扫描奇数行，即奇数场。一帧画面内首先显示的场被称为“上场”，而组成该帧画面的另一场被称为“下场” 。
...
NTSC制式（National Television Systems Committee，正交平衡调幅制），简称为N制，主要在日本、韩国及东南亚地区与美国等地区使用。帧速率为29.97fps，每帧525行262线，标准分辨率为720×480。

PAL制式（Phase-Alternative Line，正交平衡调幅逐行倒相制），又称帕尔制，主要在中国大陆、部分欧洲等国家和地区使用。帧速率为25fps，每帧625行312线，标准分辨率为720×576。

SECAM制式（Sequential Coleur Avec Memoire，行轮换调频制），又称塞康制，主要在大部分独联体国家（如俄罗斯）、法国、埃及以及非洲的一些法语系国家等使用。每秒25帧，每帧625行。隔行扫描，画面比例4:3，分辨率为720×576。

![电视制式分布](../image/Fig_1.19.png)

> 关于NTSC制式帧率非整数问题：在黑白电视的年代中，NTSC制式的帧率确实是整数。但是在制定彩色电视标准时，为了消除由彩色信号及伴音信号所产生的图像干扰，每秒帧幅由30帧稍微下调至29.97帧。也就是说，帧率整体下降了千分之一。

> NTSC每秒有30帧，与电影的24帧/秒差别太大，因此必须通过“2:3 Pull-Up”等办法把24个电影帧转成30个视频帧，这30个视频帧里所包含的内容和24个电影帧是相等的，所以NTSC的播放速度和电影一样。而PAL和电影每秒只差1帧，所以以前一般来说就直接一帧对一帧进行制作，没有经过任何处理直接被装载到DVD中去，这样PAL每秒会比电影多放一帧，因此其播放速度比电影院内或NTSC制式电视广播加快了1/24≈4%。所以，对于同一部电影来说，PAL制的DVD会比NTSC制的同一部片子快1/24。例如一部100分钟的电影，在NTSC上播放还是100分钟，而放在PAL上播放就变成96分钟了。同时声音速度加快，音调也会变高。